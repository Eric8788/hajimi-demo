...
**Current Hajimi version:** Hajimi Beta v0.2.0-beta.12 · 2026-05-22
...
- **Leaderboard:** Leaderboard only returns verified users. `/leaderboard` focuses on XP contribution views: all-time, daily, weekly, and monthly. Function Hall has a separate Hub project heat ranking based on verified unique players, capped effective opens, rating stars, and rating count. One verified user counts once per project per local day for unique players; effective opens are 30-minute sessions capped at 3 per user per day.
