...
**Current Hajimi version:** Hajimi Beta v0.2.0-beta.15 · 2026-05-22
...
- **Leaderboard:** Leaderboard only returns verified users. `/leaderboard` focuses on XP contribution views: all-time, daily, weekly, and monthly. Function Hall has separate Hub project `热度榜` and `星级榜`: heat ranks capped effective opens first, then verified unique players and rating; rating ranks cumulative stars first, then rating count while still displaying the selected day/week/month play window. One verified user counts once per project per local day for unique players; effective opens are 30-minute sessions capped at 3 per user per day.
