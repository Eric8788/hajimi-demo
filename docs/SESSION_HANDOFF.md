...
- Post cards return and display up to three most-recent liker avatars, with hydrated thumbnails showing the liker's username on hover/focus and linking to public profiles. Active, verified, non-read-only members see the target's verification name on member profiles, with reviewed grade for students; anonymous, read-only, unverified, and disabled accounts do not receive those fields. New follows create `user_follow` notifications; clicking one returns to the recipient's own profile, and unfollowing is silent.
...
- Verification `Name` means school common/preferred name, not legal name. After approval, the verification name and reviewed student grade are visible only to active, verified, non-read-only members on member profile pages; external visitors, parents, visitors, unverified accounts, and disabled accounts do not receive them. Student ID and teacher subject remain admin-only.
