...
实现口径：认证启动金 v1 由管理员在 `/admin/coins` 筛选已认证且未停用用户，批量勾选后一次性发放统一数量和备注。新认证用户统一发 10 H币；已经按旧口径收到 3 H币的老用户补发 7 H币，备注写“认证启动金补足至 10 H币”。每个用户都会写入 `coin_transactions` 账本；不新增自动防重复表，是否已发过先由管理员通过余额、上次发放记录和备注管理。
...
3. 历史 XP 是否给一次性 H币补偿。目前倾向不要大量补偿；verified 用户只做 10 H币认证启动金，已发过 3 H币的老用户补发 7 H币。
