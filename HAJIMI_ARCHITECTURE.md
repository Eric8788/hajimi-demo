...
- `/functions` **(Function Hall - Public):** 
  ...
  - The application composer highlights four required fields: project name (2–80 characters), HTTP(S) project URL, one cover image, and introduction (8–520 characters). The compact form uses two columns on desktop and one on phones; icon/tags are optional, and selecting an owned project pre-fills a new-version application. The submission API rejects missing project URLs and covers. Existing published rows and pending applications are not migrated.
  - Covers accept dropped/pasted/selected PNG, JPEG, or WebP sources up to 20 MB, with proportional 16:9 preview/export and pointer or keyboard crop adjustment. Students apply the crop before submitting; pending image edits/uploads block submission, and network errors preserve form content. Storage still uses the existing Blob upload endpoint (1 MB exported image limit).
  ...
  - The Function Hall announcement carousel contains one submission entry plus the three latest live projects with HTTP(S) URLs, ordered by `created_at` descending (ID breaks ties). Each project uses its stored cover and a direct link; primary and middle-button opens use the existing `recordProjectOpen` path. Autoplay pauses on hover/focus, hidden documents, and reduced-motion preferences. The ranking algorithm is unchanged.
