import { clamp, createRng, lerp, loopNoise } from './math'
...
export interface EyePoseOptions {
  /** 0 = spherical tangent projection, 1 = eye capsules aligned to the screen. */
  screenUpright?: number
}
...
export function eyePoses(
  ...
  const screenUpright = clamp(options.screenUpright ?? 0)

  const build = (side: number): EyePose => {
    const [ef, er] = spin(f, right, deg(split * side))
    // A gaze direction should move an eye, not rotate the eye capsule on screen.
    // Keep the projected perspective scale, but remove the tangent-plane shear
    // that makes both eyes visibly slant while the pointer moves.
    const horizontal = Math.hypot(er[0], er[1])
    const vertical = Math.hypot(down[0], down[1])
    return {
      x: ef[0] * scale,
      y: ef[1] * scale,
      a: lerp(er[0], horizontal, screenUpright),
      b: lerp(er[1], 0, screenUpright),
      c: lerp(down[0], 0, screenUpright),
      d: lerp(down[1], vertical, screenUpright),
      depth: ef[2]
    }
  }
