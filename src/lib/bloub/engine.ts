...
export interface Look {
  ...
  spin: number
  wander: number
  /** 1 keeps eye capsules upright during pointer tracking; 0 preserves expression tilt. */
  upright: number
}

const NO_LOOK: Look = { yaw: 0, pitch: 0, mix: 0, spin: 0, wander: 1, upright: 0 }
...
const lerpLook = (a: Look, b: Look, t: number): Look => ({
  ...
  upright: lerp(a.upright, b.upright, t)
})
...
export class BotEngine {
  ...
  setLook(look: Look | null, now: number, morph = BotEngine.LOOK_MORPH) {
     ...
     */
    if (look && !Number.isFinite(look.yaw + look.pitch + look.mix + look.spin + look.wander + look.upright)) {
      return
    }
  ...
  sample(now: number): BotFrame {
    ...
    if (pose.eyeAlpha > 0.01) {
      const screenUpright = clamp(look.mix * look.upright)
      const poses = eyePoses(gaze, R, pose.split, {
        // The pointer controls where the eyes look, while their screen-space
        // orientation stays upright. Catalogue animation states keep the
        // original tangent-plane projection and intentional expression tilts.
        screenUpright
      })
      for (let i = 0; i < 2; i++) {
        const e = poses[i]!
